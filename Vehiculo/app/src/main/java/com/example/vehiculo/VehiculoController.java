...
public class VehiculoController {
    ...
    private Context c;
    public VehiculoController(Application c) {
    ...
    public boolean buscarVehiculo(Vehiculo e){
        ...
        Cursor c = sql1.query(DefBD.tabla_veh,null,"codigo=?",args,null,null,null);
        if (c.getCount()>0){
            bd.close();
            return true;
        }
    ...
    public Cursor allVehiculos2(){
        try{
            ...
            Toast.makeText(c, "Error consulta vehiculo " + ex.getMessage(), Toast.LENGTH_LONG).show();
            return null;
        }
    }
    ...
    public void eliminarVehiculo(String cod){
        try{
            ...
            if (!buscarVehiculo(cod)) {
                ...
                SQLiteDatabase sql = bd.getWritableDatabase();
                sql.delete(DefBD.tabla_veh, "codigo=?", args);
                Toast.makeText(c, "Vehiculo eliminado", Toast.LENGTH_LONG).show();
            }
        }
        catch (Exception ex){
            Toast.makeText(c, "Error eliminar vehiculos " + ex.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void actualizarVehiculo(Vehiculo e){
        try{
            ...
            valores.put(DefBD.col_marca, e.getMarca());
            valores.put(DefBD.col_color, e.getColor());
            sql.update(DefBD.tabla_veh,valores,"codigo=?",args);
